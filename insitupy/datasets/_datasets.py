...
def human_breast_cancer(
    ...
    if image_dir.exists():
        ...
        if md5sum(image_dir/"slide_id__hbreastcancer__HE__histo") == expected_he_md5sum:
                if not overwrite:
    ...
    if image_dir.exists():
        print("Image exists. Checking md5sum...")
        if md5sum(image_dir/"slide_id__hbreastcancer__CD20_HER2_DAPI__IF") == expected_if_md5sum:
                if not overwrite:
                    print(f"The md5sum matches. Download is skipped. To force download set `overwrite=True`.")
                    return
                else:
...
def pancreatic_cancer(
    ...
    expected_if_md5sum = 'c859a7ab5d29807b4daf1f66cb6f506'
